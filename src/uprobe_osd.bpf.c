...
inline __u64 fetch_register(const struct pt_regs * const ctx, int reg)
{
    ...
    /* switch case is not supported well by eBPF, we'll run into  unable to deference modified ctx error 
    switch (reg) {
	case 6:
	    v = ctx->rbp;
	    break;
	...
	case 0:
	    ...
	    break;
	case 1:
	    v = PT_REGS_PARM3(ctx); //rdx
	    break;
	case 2:
	    v = PT_REGS_PARM4(ctx); //rcx
	    break;
	case 3:
	    v = ctx->rbx;
	    break;
...
int print_vf(struct VarField *vf)
{
	if (vf == NULL) 
		return 0;
	bpf_printk("reg %d offset %d onstack %d\n", vf->varloc.reg, vf->varloc.offset, vf->varloc.stack);
	if (1 >= vf->size)
		return 0;
	bpf_printk("field 1 offset %d, pointer %d", vf->fields[1].offset, vf->fields[1].pointer);
	if (2 >= vf->size)
		return 0;
	bpf_printk("field 2 offset %d, pointer %d", vf->fields[2].offset, vf->fields[2].pointer);
	if (3 >= vf->size)
		return 0;
	bpf_printk("field 3 offset %d, pointer %d", vf->fields[3].offset, vf->fields[3].pointer);
	if (4 >= vf->size)
		return 0;
	bpf_printk("field 4 offset %d, pointer %d", vf->fields[4].offset, vf->fields[4].pointer);
	if (5 >= vf->size)
		return 0;
	bpf_printk("field 5 offset %d, pointer %d", vf->fields[5].offset, vf->fields[5].pointer);
	return 0;
}
...
int uprobe_enqueue_op(struct pt_regs *ctx)
{
	...
	if (op_type != 42) {
	    bpf_printk("uprobe_enqueue_op got non osd_op type %d", op_type);
	    return 0;
	}
	...
	if(NULL != vf) {
	  ...
	  bpf_probe_read_user(&stamp.nsec, sizeof(stamp.nsec), (void *)(recv_stamp_addr+4));
	...
	if(NULL != vf) {
	  ...
	  bpf_probe_read_user(&stamp.sec, sizeof(stamp.sec), (void *)recv_complete_stamp_addr);
	  bpf_probe_read_user(&stamp.nsec, sizeof(stamp.nsec), (void *)(recv_complete_stamp_addr+4));
	...
	if(NULL != vf) {
	  ...
	  bpf_probe_read_user(&stamp.sec, sizeof(stamp.sec), (void *)dispatch_stamp_addr);
	  bpf_probe_read_user(&stamp.nsec, sizeof(stamp.nsec), (void *)(dispatch_stamp_addr+4));
...
int uprobe_dequeue_op(struct pt_regs *ctx)
{
	...
	if (op_type != 42) {
	    bpf_printk("uprobe_dequeue_op got non osd_op type %d", op_type);
	    return 0;
	}
	//read num
	++varid;
...
int uprobe_execute_ctx(struct pt_regs *ctx)
{
	...
	if(NULL != vp) { 
		...
		bpf_printk("uprobe_execute_ctx, no previous op info, owner %lld, tid %lld\n", key.owner, key.tid);
		return 0;
	}
...
int uprobe_submit_transaction(struct pt_regs *ctx)
{
	...
	bpf_printk("Entered into uprobe_submit_transaction key owner %lld, tid %lld, op %llx\n", key.owner, key.tid, vp);
	if(NULL != vp) { 
		vp->submit_transaction_stamp = bpf_ktime_get_boot_ns();
		__u64 thid = get_tid();
		bpf_map_update_elem(&tid_k, &thid, &key, 0);
	} else {
		bpf_printk("uprobe_submit_transaction, no previous op info, owner %lld, tid %lld\n", key.owner, key.tid);
	}
	return 0;
}
...
int uprobe_queue_transactions(struct pt_regs *ctx)
{
   ...
   if (NULL != key) {
	   ...
	   bpf_printk("uprobe_queue_transaction, no previous key matched owner %lld, tid %lld\n", key->owner, key->tid);
       }
   } else {
...
int uprobe_do_write(struct pt_regs *ctx)
{
    ...
    if (NULL != key) {
	   ...
	   bpf_printk("uprobe_do_write, no previous key matched owner %lld, tid %lld\n", key->owner, key->tid);
       }
    } else {
...
int uprobe_wctx_finish(struct pt_regs *ctx)
{
    ...
    if (NULL != key) {
	   ...
	   bpf_printk("uprobe_wctx_finish, no previous key matched owner %lld, tid %lld\n", key->owner, key->tid);
       }
    } else {
...
int uprobe_txc_state_proc(struct pt_regs *ctx)
{
	...
	bpf_probe_read_user(&state, sizeof(state), (void *)state_addr);
	   ...
	   bpf_printk("uprobe_txc_state_proc, no previous key matched owner %lld, tid %lld\n", key->owner, key->tid);
	}
    } else {
...
int uprobe_txc_apply_kv(struct pt_regs *ctx)
{
	...
	bpf_probe_read_user(&state, sizeof(state), (void *)state_addr);
	   ...
	   bpf_printk("uprobe_txc_apply_kv, no previous key matched owner %lld, tid %lld\n", key->owner, key->tid);
...
int uprobe_log_op_stats(struct pt_regs *ctx)
{
	...
	if(NULL != vp) { 
		...
		*e = *vp;
	        bpf_printk("Output tid %lld, execute_ctx %lld \n", key.tid, vp->execute_ctx_stamp);
		bpf_ringbuf_submit(e, 0);
	} else {
		bpf_printk("uprobe_log_op_stats, no previous op info, owner %lld, tid %lld\n", key.owner, key.tid);
	}
end:
